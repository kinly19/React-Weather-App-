...
const WeatherCard = () => {
  ...
  const backGround = { //background styling for main container
    ...
    if (locationKey) {
      fetch(
        ...
        .then((res) => {
          setError(null);
          setWeather(
            //we set the weather state by .mapping through our res (api data) and returning our own object(seen below), we only use what we need and not the whole object from api.
            res.forecast.forecastday.map((fcd) => {
              //daily forecast
              return {
                ...
                humidity: isToday
                  ...
                  : `${fcd.day.avghumidity}%`, //this is making another api call.
                feelslike: isToday
                  ? res.current.feelslike_c + degreeSign
                  : "N/A",
  ...
  useEffect(() => {
    //To change background Image, depending on time of day between sunset and sunrise
    if (weather) {
          ...
          {hourlyForecast && (
            <div className="w-card__slider">
              {hourlyForecast.map((hour, index) => {
                //we take our array and map through and return template
